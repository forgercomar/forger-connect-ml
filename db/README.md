...
docker exec -i wf-postgres psql -U wfml -d wfml < db/001_init.sql

# O desde el container del connect-ml:
node migrate.js
```
...
- Tokens encriptados con AES-256-GCM; la key vive en `WFML_TOKEN_KEY`.
...
PGPASSWORD=...
PGDATABASE=wfml
WFML_TOKEN_KEY=<32 bytes base64>     # rotable cada 3-6 meses
WFML_OAUTH_HUB_SECRET=<existing>     # shared con plugins viejos (compat)
```
