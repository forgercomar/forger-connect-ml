...
-- Modelo conceptual:
--
--   accounts          → una fila por cada conexión OAuth ML de cada cliente.
...
--   - Tokens ML (refresh) se guardan encriptados con AES-256-GCM. La key vive
--     en env var WFML_TOKEN_KEY (rotable), nunca en la DB.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- accounts: mapping cuenta ML ↔ cliente WP
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS accounts (
    id               BIGSERIAL PRIMARY KEY,
    public_id        TEXT NOT NULL UNIQUE,         -- ej. "acc_a3f9..."
    -- Datos del seller ML (lo identifica univocamente; no se duplica si el
    -- mismo seller se conecta desde dos sitios WP distintos).
    ml_user_id       BIGINT NOT NULL,
    ml_nickname      TEXT,
    ml_site_id       TEXT,                          -- "MLA", "MLM", etc.
    -- Tokens ML — el refresh_token se rota cada vez que se renueva.
    -- access_token NO se persiste: se obtiene on-demand desde refresh_token.
    refresh_token_enc TEXT,                         -- base64(iv ‖ ciphertext ‖ tag)
    refresh_token_iv  TEXT,                         -- iv en base64 (12 bytes p/ AES-GCM)
    token_expires_at  TIMESTAMPTZ,                  -- expiración cacheada del access
    -- Cliente WP que tiene esta cuenta conectada.
...
CREATE UNIQUE INDEX IF NOT EXISTS accounts_ml_user_idx
    ON accounts (ml_user_id) WHERE revoked_at IS NULL;
...
CREATE TABLE IF NOT EXISTS job_steps (
    ...
    --   push:          { "items": [{"ml_item_id":"MLA1","mode":"all"}, ...] }
    input           JSONB NOT NULL DEFAULT '{}'::jsonb,
