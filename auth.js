 ...
 *
 *   X-Wfml-Account   public_id de la cuenta (ej. "acc_a3f9...")
 *   X-Wfml-Ts        timestamp Unix (segundos) — protege contra replay
 *   X-Wfml-Sig       hex(HMAC-SHA256(secret, payload))
 *
 ...
 *
 * La key vive en env var `WFML_TOKEN_KEY` (32 bytes base64). Rotable: para rotar,
 * agregás `WFML_TOKEN_KEY_PREV`, decrypts viejos siguen funcionando hasta que
...
function getTokenKey() {
    const raw = process.env.WFML_TOKEN_KEY;
    if (!raw) {
        throw new Error('WFML_TOKEN_KEY no está definida en el entorno');
    }
    const key = Buffer.from(raw, 'base64');
    if (key.length !== 32) {
        throw new Error(`WFML_TOKEN_KEY debe ser 32 bytes (base64); actual: ${key.length} bytes`);
    }
    return key;
}
...
export function decryptToken(ciphertextB64, ivB64) {
    ...
    try {
        ...
        // Rotación de key: reintentar con WFML_TOKEN_KEY_PREV si está definida.
...
/**
 * Key anterior para rotación (WFML_TOKEN_KEY_PREV). Devuelve null si no está
 * definida o es inválida — en ese caso el fallback simplemente no se intenta.
 */
function getPrevTokenKey() {
    const raw = process.env.WFML_TOKEN_KEY_PREV;
    if (!raw) return null;
    const key = Buffer.from(raw, 'base64');
    if (key.length !== 32) {
        console.warn('[auth] WFML_TOKEN_KEY_PREV inválida (debe ser 32 bytes base64) — ignorada');
        return null;
    }
    return key;
}
